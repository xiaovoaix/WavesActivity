...
from ..util import generate_random_string, get_public_ip
...
async def get_base_header(devCode: Optional[str] = None) -> Dict[str, str]:
...
def check_send_master_info(code: int, msg: str, data: Optional[T] = None) -> bool:
    ...
    logger.warning(f"[rover-reminder] {'.'.join(callers)} code: {code} msg: {msg} data: {data}")
